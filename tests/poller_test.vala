...
class TestPoller : AlpineQaBot.Poller {
    ...
    public Gee.ArrayList<AlpineQaBot.Job> test_poll (uint project_id, Soup.Session? default_soup_session = null, string db_dir = Config.SHARED_STATE_DIR, GLib.DateTime? date = null) throws AlpineQaBot.DatabaseError {
        ...
        res.add_all (this.poll_active_merge_requests (project_id, default_soup_session, date) ?? new Gee.ArrayList<AlpineQaBot.Job>());
        res.add_all (this.poll_stale_merge_requests (project_id, default_soup_session, date) ?? new Gee.ArrayList<AlpineQaBot.Job>());
...
void test_poller_poll () {
    ...
    try {
        jobs = poller.test_poll (19765543, test_soup_session, tmp_dir.file_path, new GLib.DateTime.from_iso8601 ("2020-07-20T19:57:00Z", null));
    } catch (GLib.Error e) {
        error (e.message);
    }
